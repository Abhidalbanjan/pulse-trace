...
Baselines at kickoff: backend ~66% ready, screens ~60%. The gap between them
**was** the problem statement: several finished backends had no face. (The two
readiness-audit documents behind those numbers were retired once the gap closed —
[PARITY_REPORT.md](PARITY_REPORT.md) is the live successor, generated by CI.)
...
- ✅ CI gate ([`eval_test.go`](shared/causal/eval_test.go), runs in the `shared` job): fails the build if the deterministic accuracy regresses below thresholds (overall ≥ 85%, root-service ≥ 85%, playbook ≥ 95%). Current rule-based score: **90.9%** — published in CAUSAL_EVAL.md.
...
- ✅ **Documented DR posture** — `DISASTER_RECOVERY.md`: RPO/RTO by data tier, per-datastore backup strategy (managed in SaaS, native tooling on-prem) and ordered restore runbooks (secrets→Postgres→ClickHouse/Neo4j→app→replay), failure-scenario responses, drill cadence, and the honest single-region-today vs. deferred multi-region stance. Restore integrity ties to the F20 tamper-evident audit-chain verify.
