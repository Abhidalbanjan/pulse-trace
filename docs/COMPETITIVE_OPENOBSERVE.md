...
| D3 | Ad-hoc query power | **Closed. 6 of 6 benchmark classes expressible** (P3.1–P3.2 + aggregate push-down). SQL over a 7-relation catalog plus open-ended log attributes; the high-cardinality group-by that was unaskable in every earlier run answers in **4 ms against their 56 ms**, with `rows_scanned: 0` — the aggregate is computed in the store and only buckets travel. Historic position: no user query language at all; logs = Quickwit query string, metrics = fixed params → server-built SQL, no joins, no aggregations | **6 of 6 expressible.** **DataFusion** SQL engine + a **full PromQL engine** (aggregations/binaries/functions) + tantivy inverted index + bloom pruner + result cache | **parity on expressibility** — they retain PromQL and a result cache (our P3.4/P3.5) |
...
| D3 query power | "no query language" | **was 2 of 6 classes not expressible; now 0 of 6** — closed by P3 on 2026-08-19 |
