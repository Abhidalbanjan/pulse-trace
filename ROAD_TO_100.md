...
### F17 — Billing & revenue · 58→**100** BE, 55→**100** UI · effort L · ✅ delivered
Stripe checkout/portal + HMAC webhook + subscription lifecycle already existed; F17 completed the in-product money path.
- ✅ **In-app plan comparison** ([`billing/plans.go`](gateway-service/internal/billing/plans.go)) — pure `BuildPlanCatalog` renders every tier with per-plan **upgrade/downgrade** CTAs derived from tier order (manual/on-prem provider → all "contact"; enterprise always sales-assisted). Quota limits come straight from the enforcer ([`quota.LimitsForPlan`](gateway-service/internal/quota/quota.go)) so the pricing page can't drift; display prices overridable via `BILLING_PRICE_<PLAN>`. `GET /api/v1/billing/plans`; the [`BillingPanel`](frontend/src/components/Settings/BillingPanel.tsx) shows highlighted plan cards replacing the old hardcoded buttons.
- ✅ **Invoice history** — `ListInvoices` on the provider (Stripe `/v1/invoices` via an authenticated GET; manual → none); `GET /api/v1/billing/invoices` (empty, never error, for no-customer/on-prem); UI table (date, number, amount, status, PDF/view link).
- ✅ **Dunning / failed-payment recovery** — webhook handles `invoice.payment_failed` → recoverable **`past_due`** (still usable, Stripe retries) and `invoice.payment_succeeded/paid` → `active`; pure, tested `statusForSubscription` mapping (unpaid/expired/canceled → suspended). UI dunning banner with an **Update payment method** CTA + a `past_due` status pill.
- ↪ Deferred (depth): tax/invoicing customization and metered-usage→invoice line items (needs F16 metering-export) — the subscription money path, plan changes, invoices, and dunning are all in-product now.
- **DoD:** the self-serve money path works in-product — compare/upgrade/downgrade, invoice history, and failed-payment recovery are all first-class; R1–R7.

### F18 — Auth, RBAC/ABAC · 75/78→**100** · effort M · ✅ delivered
OAuth2/OIDC SSO already existed; F18 filled every remaining enterprise-auth gap — MFA, session revocation, password lifecycle, guided policy editing, SCIM provisioning, and SAML federation.
...
- ✅ **Guided ABAC policy-authoring UX** — the Policies tab ([`PoliciesPanel`](frontend/src/components/Settings/PoliciesPanel.tsx)) replaces the raw expr-lang text box with a **clause builder** (attribute · operator · value rows, AND/OR join, real role names + action/tier suggestions) that generates the expression, alongside an **Advanced (expr)** mode for power users. Both get **live validation**: a debounced `POST /api/v1/admin/policies/validate` compiles the condition server-side and surfaces the exact compiler error, and Create is blocked while invalid. Backend factored the compile check into a reusable [`ValidateCondition`](gateway-service/internal/auth/rbac.go) (shared by create + the new endpoint), keeping the validator and the request-time evaluator on the same attribute env.
- ✅ **Tests:** RFC-6238 vectors, skew tolerance, stale/wrong-code rejection, challenge-token round-trip + session/garbage rejection, AES round-trip + fresh-nonce non-determinism, recovery-code hashing/normalization, plus `ValidateCondition` valid/invalid coverage (20 unit tests across [totp_test.go](gateway-service/internal/auth/totp_test.go)/[mfa_test.go](gateway-service/internal/auth/mfa_test.go)/[rbac_test.go](gateway-service/internal/auth/rbac_test.go)); Playwright asserts the Security tab offers enrolment and the guided builder composes + validates a condition.
- ✅ **Session revocation & device management** — every session is recorded in `user_sessions` keyed by the token's `jti`; [`SessionStore`](gateway-service/internal/auth/sessions.go) keeps an in-memory revoked-jti cache (30s refresh + instant local apply) so `AuthMiddleware` rejects a revoked token as a map lookup, not a DB hit. Settings → Security lists devices with per-session revoke + "sign out all other devices". Migration 022.
- ✅ **Password lifecycle** — authenticated change (revokes other sessions) + anti-enumeration forgot/reset ([`password.go`](gateway-service/internal/auth/password.go)): reset tokens are `<id>.<secret>` (O(1) lookup, only `bcrypt(secret)` stored, single-use, 1h TTL); a completed reset revokes all sessions. Minimal `net/smtp` [mailer](gateway-service/internal/auth/mailer.go) with graceful degrade (logs the link) when SMTP is unset; `/login` forgot sub-flow + `/reset-password` page. Migration 023.
- ✅ **SCIM 2.0 provisioning** ([`scim.go`](gateway-service/internal/auth/scim.go), RFC 7644) — enterprise IdP directory sync over `/scim/v2/Users` (list/create/get/PUT/PATCH/DELETE, both Okta & Azure PATCH dialects), static `SCIM_TOKEN` bearer auth (constant-time, disabled 503 without a token). Deprovision = deactivate + revoke sessions; `Login` refuses a deactivated account. Migration 024.
- ✅ **SAML 2.0 SSO** ([`saml.go`](gateway-service/internal/auth/saml.go)) — a real Service Provider via the vetted `crewjam/saml` library (XML-dsig verification, audience/condition checks): metadata / AuthnRequest-redirect (request id pinned for InResponseTo) / signature-verifying ACS, then the verified identity plugs into the same JWT-session provisioning as OIDC. `/login` offers "Continue with SAML SSO"; disabled unless the IdP metadata is configured.
- **DoD:** enterprise MFA + SSO (OIDC **and** SAML) + SCIM lifecycle + session/password self-service + guided, validated policy editing — **met**.
...
### F21 — Infra, CI/CD, secrets, DR · 40–70→**100** · effort L · ✅ delivered
The Helm chart, k8s hardening (HPA/PDB/resources), ESO secret template, and the F0.2 perf harness already existed; F21 closed the CI-security, deploy-automation, and DR-documentation gaps.
- ✅ **Security scanning + CVE remediation** — new CI `security` job runs [`govulncheck`](https://golang.org/x/vuln) per Go module (gated at **zero**; it reports only call-reachable CVEs, so a hit is real). Remediated **7 code-reachable Go CVEs** across all 8 modules (grpc 1.64→1.82, otel 1.44, x/net 0.55, x/text 0.39, pgx 5.9.2, goxmldsig 1.6, quic-go 0.59.1) — every module builds/tests green and scans clean. Frontend gates on critical npm advisories + reports highs non-blocking; patched `sharp`/`postcss` transitives via `overrides`.
- ✅ **Perf-gated CI** — the [`load-test`](.github/workflows/ci.yml) job drives the ingestion hot path through k6 with baked p95<800ms / p99<1500ms / error-rate<2% thresholds on every PR; the deep multi-protocol soak runs on schedule (`scale-baseline.yml`). (Already in place — verified.)
- ✅ **Reproducible deploy with managed secrets** — `docker-build` now builds+pushes each service to **GHCR** on merge to main (image names aligned to the Helm chart), and a guarded **`deploy-staging`** job runs `helm upgrade --install` pinned to the commit sha (skips cleanly without a `STAGING_KUBECONFIG`, so it never wedges CI); verified via `helm template` that image coordinates match what's pushed. The ESO template ([`k8s/externalsecret.yaml.example`](k8s/externalsecret.yaml.example)) defines a concrete `SecretStore` (Vault, swappable to AWS/GCP/Azure) materializing the same Secret the workloads consume; secret templates synced with the F17/F18 fail-closed secrets (MFA/CHANNEL encryption keys, SCIM, SAML, Stripe).
- ✅ **Documented DR posture** — [`DISASTER_RECOVERY.md`](DISASTER_RECOVERY.md): RPO/RTO by data tier, per-datastore backup strategy (managed in SaaS, native tooling on-prem) and ordered restore runbooks (secrets→Postgres→ClickHouse/Neo4j→app→replay), failure-scenario responses, drill cadence, and the honest single-region-today vs. deferred multi-region stance. Restore integrity ties to the F20 tamper-evident audit-chain verify.
- ↪ Deferred (per plan, not built speculatively): first-class **multi-region active/standby** (cross-region streaming replication + hot failover, ~15%) — revisit at the first customer whose contractual RTO drops below the restore-based window; production-cluster soak beyond the scheduled baseline.
- **DoD:** reproducible prod deploy (Helm + GHCR + guarded staging deploy) with secrets managed (ESO, fail-closed), perf-gated CI, security-scanned at zero call-reachable CVEs, and a documented DR posture — **met**.
...
| **4** ✅ | Revenue & enterprise | F17 billing, F18 SSO/MFA/policy UX, F20 audit, F21 infra/DR | ✅ **All delivered.** ✅ F20 audit (hash-chained tamper-evidence + verify/export), ✅ F18 auth (MFA + session revocation + password lifecycle + guided ABAC builder + SCIM 2.0 + SAML SSO), ✅ F17 billing (plan compare/upgrade/downgrade + invoice history + dunning recovery), ✅ F21 infra (govulncheck CI + 7 CVEs remediated + GHCR/staging deploy + ESO secret sync + documented DR posture). Self-serve money path + enterprise procurement bar met. |
