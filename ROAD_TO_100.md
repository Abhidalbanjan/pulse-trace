...
| **4** 🔄 | Revenue & enterprise | F17 billing, F18 SSO/MFA/policy UX, F20 audit, F21 infra/DR | Self-serve money path + enterprise procurement bar met. **In progress:** ✅ F20 audit (hash-chained tamper-evidence + verify/export), ✅ F18 auth (MFA + session revocation + password lifecycle + guided ABAC builder + SCIM 2.0 + SAML SSO), ✅ F17 billing (plan compare/upgrade/downgrade + invoice history + dunning recovery). Remaining: F21 infra/CI/secrets/DR. |
