...
- ↪ **Still open (future F18 slices):** SAML/SCIM SSO, session revocation/device management, and the password-reset flow.
- **DoD:** enterprise MFA is self-serve, recoverable, and un-bypassable; **self-service policy editing** is guided and validated. Remaining SSO/session/reset items tracked above.
...
| **4** 🔄 | Revenue & enterprise | F17 billing, F18 SSO/MFA/policy UX, F20 audit, F21 infra/DR | Self-serve money path + enterprise procurement bar met. **In progress:** ✅ F20 audit (hash-chained tamper-evidence + verify/export), 🔄 F18 auth (TOTP MFA + two-step login + guided ABAC policy builder delivered; SSO/session/password-reset remain). |
