...
func msgFor(t *testing.T, entry models.LogEntry) bus.Message {
	t.Helper()
	b, err := json.Marshal(entry)
	if err != nil {
		t.Fatalf("marshal entry: %v", err)
	}
	return bus.Message{Topic: "logs", Value: b}
}
...
func TestHandle_ErrorAndFatalCreateAlerts(t *testing.T) {
	for _, lvl := range []models.LogLevel{models.LogLevelError, models.LogLevelFatal} {
		...
		if err := c.Handle(context.Background(), msgFor(t, models.LogEntry{ID: "1", ServiceName: "payments", Level: lvl, Message: "boom"})); err != nil {
...
func TestHandle_NonAlertLevelsAreIgnored(t *testing.T) {
	...
	for _, lvl := range []models.LogLevel{models.LogLevelInfo, models.LogLevelWarning, models.LogLevelDebug} {
		...
		if err := c.Handle(context.Background(), msgFor(t, models.LogEntry{ID: "1", ServiceName: "s", Level: lvl, Message: "routine"})); err != nil {
			t.Fatalf("level %s: Handle returned error: %v", lvl, err)
		}
...
func TestHandle_MalformedMessageIsSwallowed(t *testing.T) {
	...
	// A poison message must not be returned as an error (which would make the
	// bus redeliver it forever) and must not create an alert.
	err := c.Handle(context.Background(), bus.Message{Topic: "logs", Value: []byte(`{not valid json`)})
	if err != nil {
...
func TestHandle_InsertErrorIsReturnedForRetry(t *testing.T) {
	...
	// A transient store failure on a real alert must propagate so the bus retries
	// the message rather than dropping the alert.
	err := c.Handle(context.Background(), msgFor(t, models.LogEntry{ID: "1", ServiceName: "s", Level: models.LogLevelError, Message: "boom"}))
	if err == nil {
